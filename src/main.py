...
def main() -> None:
    ...
    # =========================================================================
    # 6. HTML 리포트 생성 (알림 앞 — 실패 시 fail-fast)
    # =========================================================================
    # 예외를 번역하지 않고 그대로 전파시킨다. 리포트 생성 실패는 대부분
    # 결정론적 버그(뷰모델↔템플릿 키 불일치)라 재시도·알림이 무의미하고,
    # 여기서 죽여야 깨진 리포트를 가리키는 링크가 알림으로 나가지 않는다.
